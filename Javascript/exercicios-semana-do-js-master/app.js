...
const numerosImpares = randomNumbers.filter(numero => numero%2===1);
...
const quantidadeDeNumerosAbaixo501 = crazyNumbers.reduce((cont, number)=> {
  if(number < 501)
    cont ++;
...
console.log(quantidadeDeNumerosAbaixo501)
/*
  03
...
const numbersAoQuadrado = numbers.map(number => number **2);
...
const filmeLançadosAntesDe2000 = tarantinoMovies.filter(filme => filme.release < 2000)

console.log(filmeLançadosAntesDe2000)
/*
  05
...
const serieNome = tvShows.map(serie => serie.name)
